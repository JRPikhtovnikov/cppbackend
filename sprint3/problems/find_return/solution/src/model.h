...
public:
    ...
    void AddLoot(int type, Vec2 pos) {
        loot_objects_[next_loot_id_++] = LostObject{next_loot_id_ - 1, type, pos};
    }
...
public:
    ...
    bool AddLoot(uint32_t loot_id, int type) {
        if (bag_.size() >= bag_capacity_) return false;
        bag_.push_back({loot_id, type});
        return true;
    }
