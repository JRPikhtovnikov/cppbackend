...
private:
    ...
    void AdvanceGameTime(std::int64_t time_delta_ms) {
        ...
        // 3. Обрабатываем коллизии (сбор предметов и возврат на базу) для каждой сессии
        for (const auto& [sid, _] : sessions_.GetAllSessions()) {
            ...
            // Получаем значения предметов для этой карты
            ...
            // Собираем все объекты для проверки коллизий: офисы и потерянные предметы
            struct CollisionObject {
                bool is_office;
                size_t office_idx;      // индекс офиса в карте (для идентификации)
            ...
            std::vector<bool> alive;      // жив ли объект (для предметов)
            ...
            for (size_t i = 0; i < offices.size(); ++i) {
                ...
                alive.push_back(true); // офисы всегда "живы"
            }
            ...
            for (const auto& [id, obj] : loot_map) {
                objects.push_back(CollisionObject{
                    ...
                    .width = 0.0  // предметы имеют нулевую ширину
                });
                alive.push_back(true);
            }

            // Собиратели (игроки данной сессии)
            struct GathererInfo {
            ...
            std::vector<model::Player*> session_players; // для быстрого доступа по индексу
            ...
            // Провайдер для FindGatherEvents
            ...
            for (const auto& ev : events) {
                if (!alive[ev.item_id]) continue; // объект уже уничтожен (предмет подобран)
                ...
                if (obj.is_office) {
                    // Игрок достиг офиса (базы) — сдаём все предметы из рюкзака
                    ...
                    if (player->GetBag().size() < player->GetBagCapacity()) {
                        // Проверяем, что тип предмета в допустимых пределах и есть значение
                        int value = 0;
                        ...
                        if (player->AddLoot(obj.loot_id, obj.loot_type, value)) {
                            // Успешно подобран — удаляем с карты
        ...
        // 4. Генерация нового лута с правильными значениями
        for (const auto& [sid, _] : sessions_.GetAllSessions()) {
            auto* session = sessions_.Find(sid);
            if (!session) continue;
            const auto* map = game_.FindMap(session->GetMapId());
