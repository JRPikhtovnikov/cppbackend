...
private:
    ...
    void AdvanceGameTime(std::int64_t time_delta_ms) {
        ...
        std::cout << "\n=== AdvanceGameTime START ===" << std::endl;
        std::cout << "time_delta_ms: " << time_delta_ms << std::endl;
        
        const double dt = static_cast<double>(time_delta_ms) / 1000.0;

        // Обновление позиций собак
        auto& all = players_.GetAllMutable();
        std::cout << "Players count: " << all.size() << std::endl;
        
        for (auto& [pid, p] : all) {
            std::cout << "  Processing player " << pid << std::endl;
            
            const auto* session = sessions_.Find(p.GetSessionId());
            if (!session) {
                std::cout << "    ERROR: session not found for player " << pid << std::endl;
                continue;
            }
            ...
            if (!map) {
                std::cout << "    ERROR: map not found for session " << session->GetId() << std::endl;
                continue;
            }
            ...
            if (v.x == 0.0 && v.y == 0.0) {
                std::cout << "    Speed is zero, skipping movement" << std::endl;
                continue;
            }
        ...
        // Генерация лута для всех сессий
        const auto& all_sessions = sessions_.GetAllSessions();
        
        for (const auto& [sid, _] : all_sessions) {
            auto* mutable_session = sessions_.Find(sid);
            if (!mutable_session) {
                continue;
            }

            const auto* map = game_.FindMap(mutable_session->GetMapId());
            if (!map) {
                continue;
            }

            unsigned looter_count = mutable_session->GetPlayers().size();
            unsigned loot_count = mutable_session->GetLootCount();

            try {
                unsigned new_loot = mutable_session->GenerateLootCount(
                ...
                if (new_loot > 0 && map->GetLootTypesCount() > 0) {
                    ...
                    for (unsigned i = 0; i < new_loot; ++i) {
                        ...
                        mutable_session->AddLoot(type, pos);
                    }
                }
            } catch (...) {}
        }
